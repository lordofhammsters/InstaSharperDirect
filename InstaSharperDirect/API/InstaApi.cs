...
namespace InstaSharper.API
{
    public class InstaApi : IInstaApi
    {
        ...
        public async Task<IResult<InstaUser>> GetUserByIdAsync(string pk)
        {
            ValidateUser();
            ...
            if (response.StatusCode == HttpStatusCode.OK)
            {
                //var user = JsonConvert.DeserializeObject<InstaUser>(json);
                //if (user == null)
                //{
                //    var errorMessage = $"Can't find this user: {pk}";
                //    _logger?.Write(errorMessage);
                //    return Result.Fail<InstaUser>(errorMessage);
                //}
                ////var converter = ConvertersFabric.GetUserConverter(user);
                //return Result.Success(user); // converter.Convert()
        ...
        public async Task<IResult<InstaUserShortList>> GetMediaLikersAsync(string mediaId, int maxPages = 0)
        {
            ValidateUser();
            ValidateLoggedIn();
            try
