...
namespace InstaSharper.API.Processors
{
    public class UserProcessor : IUserProcessor
    {
        ...
        public async Task<IResult<InstaCurrentUser>> GetUserByIdAsync(string pk)
        {
            try
            {
                var userUri = UriCreator.GetUserByIdUri(pk);
                var request = HttpHelper.GetDefaultRequest(HttpMethod.Get, userUri, _deviceInfo);
                ...
                var users = JsonConvert.DeserializeObject<InstaUserByPkResponse>(json);

                if (users.User == null)
                {
                    var errorMessage = $"Can't find this user: {users.User}";
                    _logger?.LogInfo(errorMessage);
                    return Result.Fail<InstaCurrentUser>(errorMessage);
                }
                
                var converter = ConvertersFabric.Instance.GetCurrentUserConverter(users.User);
                return Result.Success(converter.Convert());
            }
            catch (Exception exception)
            {
