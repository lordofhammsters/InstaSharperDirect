...
namespace InstaSharper.Helpers
{
    internal class UriCreator
    {
        ...
        public static Uri GetUserByIdUri(string pk)
        {
            Uri instaUri;
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.GET_USER_BY_ID, pk), out instaUri))
                throw new Exception("Cant create get user by id URI");
        ...
        {
            Uri instaUri;
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.ALLOW_MEDIA_COMMENTS, mediaId),
                    out instaUri))
